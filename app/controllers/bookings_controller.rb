  ...
  def update
    ...
    @booking.update({ confirmed: params[:confirmed] })
      ...
      if @booking.confirmed == "yes"
        TwilioClient.new.send_text("Hello Edu, Your booking for #{@booking.timeslot.offer.title} was confirmed, Chat with the Offer Owner for more details.")
      else
        TwilioClient.new.send_text("Hello Edu, Your booking for #{@booking.timeslot.offer.title}, check in wabisabi.lol new offers")
      end
